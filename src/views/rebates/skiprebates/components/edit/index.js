...
export default {
            ...
            if (this.isEdit) {
                ...
                if (this.form.configType === 3) {
                    if (this.form.customerGradeFlag === 0) {
                        this.getIntroConfigGoodModel(this.form.orderGradeID)
                    }
                }
                else {
                    if (this.form.customerGradeFlag === 0) {
                        this.getIntroConfigBaseModel(this.form.orderGradeID)
                    }
                }        
            }
            else {
    ...
    methods: {
        ...
        setdataStatus(flag, id) {
            // 如果还没有设置保存用户管理
            if (this.form.distributorIntroConfigID === '' || this.form.distributorIntroConfigID == null) {
                this.$message({
                    message: '请先设置并保存用户管理',
                    type: 'warning'
                })
        ...
        setGoodDataStatus(id) {
            ...
            this.goodsModel.forEach((item, index) => {
                ...
                // 把数组转化成接口所需要的数据接口
                goodModel.distributorIntroConfigID = item.distributorIntroConfigID
            })
            return goodModel
        },
        // 返回
        ...
        // 获取返点设置的基础模型
        getIntroConfigBaseModel(orderGradeID) {
            if (this.form.distributorIntroConfigID !== '') {
                // 编辑的时候
                if (this.isEdit) {
                    ajax.getIntroConfigBaseModel(this.form.distributorIntroConfigID,orderGradeID).then((result) => {
                        // 根据层级来排序
                ...
                // 新增的时候
                    ...
                    this.baseModel.distributorIntroConfigID = this.form.distributorIntroConfigID
                }
            }
        },
        // 获取返点设置的商品模型
        getIntroConfigGoodModel(orderGradeID) {
            if (this.form.distributorIntroConfigID !== '') {
                ajax.getIntroConfigGoodsModel(this.form.distributorIntroConfigID, orderGradeID).then((result) => {
        ...
        handleChange(activeNames) {
            ...
            if (this.form.configType === 3) {
                ...
                this.getIntroConfigGoodModel(activeNames)
            }
            else {
                this.gradeId = activeNames
                this.getIntroConfigBaseModel(activeNames)
            }
        },
        // 保存返点规则
        saveGroom(id) {
            ...
            if (model.distributorIntroConfigID === '' || model.distributorIntroConfigID == null) {
                this.$message({
                    message: '请先设置用户管理!',
                    type: 'warning'
                })
            ...
            ajax.saveIntroConfigItemBaseModel(model).then(() => {
                this.$message({
                    message: '保存成功!',
                    type: 'success'
                })
        ...
        //获取跨级推荐返点等级列表 
        getSkipIntroRebate() {
            ajax.getSkipIntroRebate(this.form.distributorIntroConfigID).then((result) => {
